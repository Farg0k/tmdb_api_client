...
class AuthenticationService extends BaseTmdbService {
  ...
  Future<RequestToken> createRequestToken({Map<String, String>? queryParameters}) async {
    final jsonResponse = await get('authentication/token/new', queryParameters: queryParameters);
    return RequestToken.fromJson(jsonResponse);
  }

  /// 2. Creates a fully authorized session ID.
  Future<Session> createSession(String requestToken, {Map<String, String>? queryParameters}) async {
  ...
  Future<GuestSession> createGuestSession({Map<String, String>? queryParameters}) async {
    final jsonResponse = await get('authentication/guest_session/new', queryParameters: queryParameters);
    return GuestSession.fromJson(jsonResponse);
  }

  /// 4. Deletes an existing session.
  Future<bool> deleteSession(String sessionId, {Map<String, String>? queryParameters}) async {
  ...
  Future<Session> createSessionFromV4(String accessTokenV4, {Map<String, String>? queryParameters}) async {
    final jsonResponse = await post(
  ...
  Future<bool> validateKey({Map<String, String>? queryParameters}) async {
    try {
      final jsonResponse = await get('authentication', queryParameters: queryParameters);
      return jsonResponse['success'] == true;
    } catch (e) {
      return false;
    }
